...
function walk.draw(c, camera)
    ...
    if sp then
        ...
        if p.secondary and p.secondary.breast then
            local t_breast = sp.bob and sp.bob.velocity * 0.012 or 0
            sp.breast_l:update(dt, -t_breast, 8, 5)
            sp.breast_r:update(dt,  t_breast, 8, 5)
        end
    end
    ...
    -- shoulder_rotate is in RADIANS — t_sway is in world px (up to ~10).
    -- multiply by 0.04 to convert px magnitude to a sane radian range (~0.06 rad max).
    -- without this, 9.5 * 0.15 = 1.4 rad = 80° shoulder rotation which swamps the hips.
    local shoulder_rotate = -t_sway * cnt * 0.04
